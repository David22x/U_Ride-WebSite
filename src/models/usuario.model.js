...
const Usuario = sequelize.define('Usuario', {
  ...
  estado:      { type: DataTypes.ENUM('activo','suspendido','inactivo'), defaultValue: 'activo' },
  verificado:  { type: DataTypes.BOOLEAN, defaultValue: false },
  advertencias:{ type: DataTypes.INTEGER, defaultValue: 0 },
}, {
  tableName: 'usuarios',
  hooks: {
    beforeCreate: async (u) => { u.contrasena = await bcrypt.hash(u.contrasena, 12); },
    beforeUpdate: async (u) => {
      if (u.changed('contrasena')) u.contrasena = await bcrypt.hash(u.contrasena, 12);
    },
  },
});
